...
> I think that we have the ability to use words—---whether
  pre-existing words or new words—--in such a way that their
  metasemantics is not monomaniacally governed by charity.
  Theoretical terms in science are commonly, and more or less
  self-consciously, used in this way.  What I propose is that
  ontologists do the same with quantifiers.  They should stipulate
  that their quantifiers are to be understood as theoretical terms
  (and so are not subject to the same level of metasemantic pressure
  from charity that governs terms like 'sofa' and 'game') that stand
  for whatever joint-carving notion is in the vicinity. (9)
