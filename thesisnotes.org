...
4. The sorities cases involve an 'ordinary thing' (a rock, say, or a
...
7. Because we are supposing that we can discover the semantics of
   'red' through this sort of introspection, we must conclude that
   there _is_ no point at which 'red' ceases to apply.  If it applies
   to the first patch (which it clearly does), then it applies to the
   next patch, because the next patch is indistinguishable.  If it
   applies to _that_ patch, then...
...
***** ways out
1. How do we avoid this conclusion?  Clearly, one or both of the
   theses of the governing view are false.  Either certain terms do
   not have determinate semantic rules, or their rules are not
...
3. One reason why we should consider this a paradoxical (or at least
   ...
   apparently incoherent terms.  See [[competence]].
