...
ontological language, it seems analogous to my introduction of
...
metaphysical semantics for the predicates of ordinary physical objects,
...
But the failure of any physicalist-reductionist program so far should
make us suspicious of this optimism.  Positivists of the early
twentieth-century attempted to reduce all `non-physical' propositions
to equivalent `physical' ones, but ``purely physical truths equivalent
in meaning or even just logically equivalent to each of the
nonphysically expressed things we believe were simply not to be
found'' \citep[59]{stroud2000a}.  Sider's worldview is not purely
physical, for he includes sets.  But this does not seem to be enough.

Take the word `cat', for instance.  This word is not a physical thing,
nor is it a set of physical things.  Word {\em tokens}---uttered or
written expressions of the word---are perhaps physical things, but
word {\em types} are not.  Sider claims that word tokens are identical
to sets:

\begin{squote}
It is perhaps natural to identify a `linguistic atom' (symbol, for a
formal language; word, morpheme, phoneme, or whatever, for a spoken
language) with a set of spacetime points: the set of points located
inside some production (inscription or utterance) of that symbol
\citeyearpar[295]{sider2011d}.
\end{squote}

But even if this is plausible, it only accounts for word tokens.
Facts about tokens of `cat' might be reducible to facts about sets of
spacetime points, but facts about word {\em types} will not be.  For
example, it is true that `cat' has three letters, or that `cat' is a
count noun.  How can these true fact be translated into the
`vocabulary' of physics, logic, and set theory?

I think similar problems will arise elsewhere\,\ldots [economics is
  one Sider mentions]

Whether or not there is any specific ``in-principle barrier'' to the
sort of physical-reductionist program that Sider advocates, the lack
of promise it has so far displayed tells against it.  We should not
think that the world described by physics, logic and set theory is the
only world there is, or even the most `fundamental' description of the
world.  For it does not fully describe the world.
