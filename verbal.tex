...
`joint-carving' notions are `in the vicinity', Sider hopes to evade
Hirsch's argument from interpretive charity.  The idea is that charity
can be put on hold.  Sider hopes that when {\em he} says ``there are
no chairs'', it will be true (if it is true) only because a
``joint-carving'' quantifier does not range over chairs.
...
\stage{Ontologists}{(in unison)}{We do not intend to use `there exist'
  such that it is obviously true that there exist chairs.  Rather, we
  intend to use `there exist' such that it is an open question whether
  or not there exist chairs.  Let our uses of the quantifier pick out
  whatever `joint-carving' notion is in the vicinity.}
...
That would be analogous to me stipulating I will use `Heeriet' to
...
Sider's proposal is ``wholly mysterious''.  But this objection fails
because English quantifiers are not restricted. \\
...
\stage{Mad Scientist}{(runs in)}{I've done it!  I've discovered the
  secret of immortality!  This child will be the first to be cured of
  Death!}

\stage{Mother}{}{You're not going to die \{ever\}!}
