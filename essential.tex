...
\begin{enumerate}[ref=\arabic*]
  ...
  %% \item $x$ constitutes at $t$ a successive chair $=_{df}$ There are a
  %%   $y$ and a $t^{\prime}$ such that $y$ is other than $x$ and $x$
  %%   constitutes at $t$ the same chair that $y$ constitutes at
  %%  $t^{\prime}$. \label{suc4}
\end{enumerate}
...
%% The criteria for being a chair (and so the `persistence conditions' of
%% sucessive chairs) are most likely looser than the criteria for being a
%% molecule, for example.  There are borderline cases of chairs, and we
%% can more or less {\em decide} whether or not the thing is a chair.
%% But we can do no such thing with a molecule.  This may simply be due
%% to the fact that the (conventional!) criteria for molecules are
%% stricter.  With chairs, we appeal to the appearance and function of an
%% alleged chair; with molecules the process is more precise:
...
%% chairs, statues and lumps are, in fact, identical with sumes.  But
%% our cross-temporal talk about them is different; for example, which
%% sum is identical with a given animal at a given time---which
%% function is an appropriate `sea lion' function---depends on the
%% `conventions' of the biological sciences.\\
...
%% Second, we are fooled by the {\em causal} relations between
%% successive chairs.  For example, if I hit the chair in the sitting
%% room with a poker and a bit of wood flies away, it is a different
%% chair in the sitting room.  By striking the one chair and sending
%% part of it flying, I cause {\em that} sum to no longer meet the
%% criteria for being designated by `chair' (chairs are not nearly so
%% scattered).  I have also therefore caused a different sum to meet
%% the criteria for being designated by `chair'.  I have caused us to
%% shift our reference from one sum to the other;
