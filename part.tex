...
(Here I am assuming that relations like $R$ are not fixed polyadic
relations.  That is, there is not one relation $R$ that can apply to
three things---the schema being $Rxyz$---and a different relation
$R^{\prime}$ that can apply to two things---$Rxy$.  Rather, I am
assuming that relations like $R$ have a single variable `slot' that
...
Fine may reply that `passenger' is ambiguous; we sometimes use it to
refer to people, who think, and other times we use it to refer to the
``qua-object'' that is the
person-under-the-description-of-being-a-passenger.  If this
``qua-object'' can be said to think, it is only because the property
of thinking is `lifted' from the actual person.  Strictly speaking,
however, the ``qua-object'' cannot think.

Were he to make this reply, Fine would not be committed to there being
co-located thinkers, but he would presumably have to accept another
bizarre consequence; that strictly speaking, passengers cannot think.

This seems to be the result of Fine's treating a single object under
different descriptions as {\em different objects}.  If I become a
...
This strange consequence---that passengers cannot think---coupled with
the `explosion of reality' is cause for concern.  But it is not
limited to our first theory.  The second theory, as we will see,
results in a similar explosion.
...
\section{Groups, sets and sums}
\label{group}
Fine's theory gives us the resources to define new composition
operators, and so new {\em kinds} of things.  In section
\ref{plural-ref} I suggested that families, crews, and other {\em
  groups} are, strictly speaking, things.  We will therefore examine
the prospects of defining a composition operator for groups---a
`group-builder'.
...
number of different times $t$, or we might say that there is a {\em
  different} composition operator at each time $t$.  I will suppose
...
\label{group-id}
There is a problem with the group operator, however.  As Uzquiano
points out, a set of people can compose more than one group at a time.
...
on Judicial Ethics as of a certain time'' \citep[151]{uzquiano2004a}.
It seems false to say that, in 2004, the Supreme Court was identical
with the Special Committee.  But if the Supreme Court, $G$, is $\sum
_{t} ( S )$ and the Special Committee, $C$, is also $\sum _{t} ( S )$,
then how can we deny that $G = C$?
...
face the same problems that plagued the group operator (section
...
the lump the lump-builder $\sum _{lump}$.  In section \ref{group-id}
we saw that for the Supreme Court and the Special Committee to be the
products of different operators means that they are different {\em
  kinds} of things---to say that both are `groups' is simply to bring
two heterogeneous kinds under one label.  Likewise, the statue and the
...
One powerful reason to resist this conclusion is that it seems to
leave open the possibility of co-located thinkers.  Fine's theory of
embodiments (section \ref{fine-h}) allowed for `qua-objects' such as
people-as-passengers, which are objects co-located with people.  Since
passengers are clearly thinking beings, we are committed to there
being multiple (possibly many) thinking beings that not only share the
same brain but overlap completely.

This problem may result from Fine's theory of composition operators as
well.  What is to stop us from defining an operator for airline
passenger?  We can assign it formal and material principles (section
\ref{material}) corresponding to the characteristics of airline
passengers.  It will produce objects co-located with people, objects
that clearly also think and talk.  If this is in fact a consequence of
Fine's theory, it is unacceptable.

In section \ref{set-id}, therefore, I will re-examine the thesis that
